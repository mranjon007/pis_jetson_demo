...
class PhoneAnswerEvent(BaseEvent):
    def __init__(
        self,
        ptype: PassengerType,
...
class PhoneAnswerEventProcessor(BaseEventCallbackProcessor):
    ...
    def __init__(
        ...
        """
        super(PhoneAnswerEventProcessor, self).__init__()
    ...
    def detect_criteria(
        ...
        Args:
            phones (List[DetectionItem]): Candidate objects for match and identify event
        ...
        """
        distances = [head.abs_distance(phone) for phone in phones]
        if not distances or min(distances) > self.distance_thresh:
            return False

        matched_phone = phones[argmin(distances)]
