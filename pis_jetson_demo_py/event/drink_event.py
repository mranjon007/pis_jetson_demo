...
class DrinkEventProcessor(BaseEventCallbackProcessor):
    LOG_HEADER: str = "[PhoneAnswer] "
    ...
    def __init__(
        ...
        """Processes phone answering event.
        Calculate distance between cellphone and person's head center
        ...
        """
        super(DrinkEventProcessor, self).__init__()
        ...
        self.drink_class_ids = ClassNamesManager.get_instance().get_drink_class_ids()
    ...
    def update(self, all_detections: List[DetectionItem]):
        ...
        drinks: List[DetectionItem] = [
            det
            for det in all_detections
            if (det.class_id in self.drink_class_ids and det.tracker_id is not None)
        ]

        if not (heads and persons and drinks):
            self.ttc_counter += 1
        else:
            head = largest_volume(heads)
            person = find_person_from_head(head, persons)
            if self.detect_criteria(drinks, head, person):
                logger.debug(
    ...
    def detect_criteria(
        ...
        Args:
            drinks (List[DetectionItem]): Candidate objects for match and identify event
        ...
        """
        distances = [head.abs_distance(drink) for drink in drinks]
        if not distances or min(distances) > self.distance_thresh:
            return False

        matched_drink = drinks[argmin(distances)]
        self.event_window.append(
            DrinkEvent(
                ptype=self.ptype,
                det_drink=matched_drink,
                det_head=head,
                det_person=person,
            )
        )
